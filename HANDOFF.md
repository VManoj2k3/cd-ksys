...
## GPU-verified Phase-2 RAG detection — ALL 5 LANGUAGES (run 12, P100, Qwen2.5-Coder-14B)
One REAL guideline PDF ingested per language (ACME standards for C [2 pages],
Python, Java, TypeScript, C++), 13 scenarios through the live HTTP API with the
RAG toggle on (default `hash` embedder). RAG SMOKE: PASS.
- languages covered: Python, C, C++, Java, TypeScript — every one detected its
  violation and cited the right PDF/rule
- violations detected: 7/7 violating scenarios produced >=1 correct finding
- citations to the correct PDF: 11/11 findings (rule text + source + page)
- false positives on the 5 compliant files + wrong-collection case: 0
- validated inline fixes generated by the model: 2 (e.g. strcpy -> bounded)
- cross-collection attribution held (both/all PDFs selected -> C or TS finding
  cites its OWN PDF); retrieval faithfulness held (C bug, only Python PDF
  selected -> silent); toggle OFF -> no guideline layer.
- HONEST recall nuance: on files that break MULTIPLE rules at once, the real
  model usually reports ONE (the most salient) — C 2/2, but Py 1/3, Java 1/3,
  TS 1/3, C++ 1/3 when a single PDF is selected. Yet S14 (same TS bug with ALL
  5 collections selected) returned 3/3 — so the ceiling is higher and this is a
  consistency/prompt issue, not a capability gap. Levers: tune the guideline
  prompt to enumerate EVERY violation in the chunk; the `llama` embedder
  strengthens retrieval of less-lexically-overlapping rules. Same as Phase 1:
  high precision, tunable recall. (Same run: Phase-1 LLM recall 30/31,
  deterministic 6/6, ACCURACY EVAL PASSED.)
