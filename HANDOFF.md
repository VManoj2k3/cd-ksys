...
## GPU-verified Phase-2 RAG detection (run 11, P100, real Qwen2.5-Coder-14B)
Two REAL guideline PDFs ingested (ACME Secure-C standard, 2 pages + ACME Python
guidelines), 7 scenarios driven through the live HTTP API with the RAG toggle
on (default `hash` embedder). RAG SMOKE: PASS.
- violations detected: 4/4 violating scenarios produced >=1 correct finding
- citations to the correct PDF: 6/6 findings (rule text + source + page; the
  goto finding correctly cited CTRL-04 on PAGE 2)
- false positives on compliant / wrong-collection code: 0
- validated inline fixes generated by the model: 2 (e.g. strcpy -> bounded)
- cross-collection attribution held (both PDFs selected -> C finding cites the
  C PDF); retrieval faithfulness held (C bug, only Python PDF selected ->
  silent); toggle OFF -> no guideline layer.
- HONEST recall gap: on the Python file that violated THREE rules at once
  (print + eval + bare TODO) the real model reported only PYLOG-01 (print) and
  missed PYSEC-02 (eval) and PYDOC-03 (TODO). Same as Phase 1: high precision,
  not exhaustive recall. Levers to close it: the guideline prompt can push
  "report EVERY violation in the chunk", and the `llama` embedder improves
  retrieval of the less-lexically-overlapping rules. (Same run: Phase-1 held at
  LLM recall 30/31, deterministic 6/6, ACCURACY EVAL PASSED.)
